import React from "react"

export const Hero = () => {
  return (
    <main className="mt-16 mx-auto max-w-7xl px-4 sm:mt-24">
      <div className="text-center">
        <h1 className="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl">
          <span className="block xl:inline">Discover the</span>{" "}
          <span className="block text-orange-600 xl:inline">
            LED Beanie Hat
          </span>
        </h1>
        <p className="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">
          Keep your head warm and your path bright with our high-quality LED
          Beanie Hat. Perfect for outdoor activities, late-night runs, and much
          more.
        </p>
        <div className="mt-5 max-w-md mx-auto">
          <a
            href="/products"
            className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-orange-600 hover:bg-orange-700 md:py-4 md:text-lg md:px-10"
          >
            Shop now
          </a>
        </div>
      </div>
    </main>
  )
}
